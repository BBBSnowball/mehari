...
class TaskOnce(shovel.Task):
    done_task_results = dict()

    @classmethod
    def should_run_task(cls, task):
        return task.fullname not in cls.done_task_results

    @classmethod
    def mark_task_done_and_save_result(cls, task, result):
        cls.done_task_results[task.fullname] = result

    def __call__(self, *args, **kwargs):
        if self.should_run_task(self):
            result = super(TaskOnce, self).__call__(*args, **kwargs)
            self.mark_task_done_and_save_result(self, result)
            return result
    ...
    def make(cls, obj):
        try:
            t = cls(obj)
            previous = cls.find(t.fullname)
            if previous:
                shovel.logger.warn('Task "%s" redefined' % t.fullname)
                shovel.logger.warn('\tPrevious definition in %s at %i' % (previous[0].file, previous[0].line))
                shovel.logger.warn('\tNew      definition in %s at %i' % (t.file, t.line))
            ...
            shovel.logger.exception('Unable to make task for %r because of %s' % (obj, e))

def task_once(f):
    return TaskOnce.make(f)
...
def source_shell_file(file):
    ...
    for line in env.split("\n"):
        ...
        if key not in os.environ or os.environ[key] != value:
            shovel.logger.debug("changed or new env var: %s=%s" % (key, value))
...
def check_libc():
    ...
    if not libc_dir.isdir():
        shovel.logger.error("$libc_dir must be set to the install directory of libc, but it doesn't exist")
        sys.exit(1)
...
def long_operation(cmd):
    if int(from_env("DEBUG_SKIP_EXPENSIVE_OPERATIONS", "0")) > 0:
        shovel.logger.info("Skipping: " + str(cmd))
    else:
        sh(cmd)
...
def cd_verbose(*dir):
    shovel.logger.info("In directory: " + str(dir))
    Path(*dir).chdir()
...
def build_reconos():
    ...
    shovel.logger.info("Not setting version")  #DEBUG
...
def update_nfsroot():
    ...
    if res != 0:
        shovel.logger.warn("WARNING: NFS root directory %s contains additional files!" % NFS_ROOT)
